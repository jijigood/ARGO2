#!/usr/bin/env python
"""
实验1: 检索成本影响 - 1.5B Pilot测试版本
============================================
使用Qwen2.5-1.5B模型进行pilot测试（速度更快）
...
Pilot测试配置:
- 模型: Qwen2.5-1.5B-Instruct (单卡即可)
- 题目: 10题 (small) 或 1000题 (full)
- 预计时间: small ~5分钟, full ~10小时
...
class RealCostImpactExperiment:
    ...
    def __init__(
        ...
        llm_model_path: str = "/data/user/huangxiaolin/ARGO/RAG_Models/models/Qwen2.5-1.5B-Instruct",
        ...
        Args:
            ...
            llm_model_path: Qwen模型本地路径 (默认1.5B用于pilot测试)
        ...
        if test_mode == "small":
            ...
            self.n_test_questions = 1000  # Pilot测试: 1000题
            self.n_cost_steps = 10
            self.mode_desc = "完整pilot测试 (1000题, 1.5B模型, 预计10小时)"
        else:
...
def main():
    ...
    experiment = RealCostImpactExperiment(
        llm_model_path="/data/user/huangxiaolin/ARGO/RAG_Models/models/Qwen2.5-1.5B-Instruct",
