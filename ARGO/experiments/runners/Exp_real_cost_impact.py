...
class RealCostImpactExperiment:
    ...
    def __init__(
        ...
        n_test_questions: int = 50,  # 减少到50题(真实LLM很慢)
        difficulty: str = "hard",    # 改为Hard难度
        seed: int = 42,
        ...
        self.gpu_ids = gpu_ids if gpu_ids else list(range(min(4, self.n_gpus)))  # 默认使用4个GPU
...
Answer with only the number (1, 2, 3, or 4):"""
    ...
    def evaluate_all_policies(self, c_r: float, theta_cont: float, theta_star: float) -> Dict:
        ...
        for i, question in enumerate(self.test_questions, 1):
            ...
            # Always-Retrieve
            result = self.simulate_always_retrieve_policy(question, c_r)
    ...
    def run_experiment(
        ...
        print(f"开始实验 - 检索成本影响")
        print(f"{'='*80}")
        print(f"c_r范围: {c_r_values[0]:.3f} ~ {c_r_values[-1]:.3f} (扫描 {n_steps} 个点)")
        ...
        for i, c_r in enumerate(c_r_values, 1):
            print(f"\n[{i}/{n_steps}] c_r = {c_r:.4f} ({c_r/c_p:.1f}x c_p)")
            print(f"{'-'*80}")
    ...
    def save_results(self, output_dir: str = "draw_figs/data"):
        ...
        filename = f"exp1_real_cost_impact_{timestamp}.json"
    ...
    def plot_results(self, output_dir: str = "figs"):
        ...
        for policy in ['ARGO', 'Always-Retrieve', 'Always-Reason']:
            ...
            plt.plot(c_r_values, quality, marker='o', label=policy, linewidth=2)
        
        plt.xlabel('Retrieval Cost (c_r)', fontsize=12)
        plt.ylabel('Average Quality', fontsize=12)
        plt.title('实验1: 质量 vs 检索成本 (真实LLM)', fontsize=14, fontweight='bold')
        plt.legend()
        plt.grid(True, alpha=0.3)
        plt.tight_layout()
        
        fig1_path = os.path.join(output_dir, 'exp1_real_cost_vs_quality.png')
        ...
        for policy in ['ARGO', 'Always-Retrieve']:
            ...
            plt.plot(c_r_values, retrievals, marker='o', label=policy, linewidth=2)
        
        plt.xlabel('Retrieval Cost (c_r)', fontsize=12)
        plt.ylabel('Average Retrievals', fontsize=12)
        plt.title('实验1: 检索次数 vs 检索成本 (真实LLM)', fontsize=14, fontweight='bold')
        plt.legend()
        plt.grid(True, alpha=0.3)
        plt.tight_layout()
        
        fig2_path = os.path.join(output_dir, 'exp1_real_cost_vs_retrievals.png')
        ...
        for policy in ['ARGO', 'Always-Retrieve', 'Always-Reason']:
            ...
            plt.plot(c_r_values, accuracy, marker='o', label=policy, linewidth=2)
        
        plt.xlabel('Retrieval Cost (c_r)', fontsize=12)
        plt.ylabel('Accuracy', fontsize=12)
        plt.title('实验1: 准确率 vs 检索成本 (真实LLM)', fontsize=14, fontweight='bold')
        plt.legend()
        plt.grid(True, alpha=0.3)
        plt.tight_layout()
        
        fig3_path = os.path.join(output_dir, 'exp1_real_cost_vs_accuracy.png')
