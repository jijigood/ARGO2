...
# 检查基准数据
echo "[1] 检查基准数据..."
if [ -d "$WORK_DIR/ORAN-Bench-13K/Benchmark" ]; then
    echo "✓ 基准数据已就绪"
    echo "  - Easy: $(wc -l < $WORK_DIR/ORAN-Bench-13K/Benchmark/fin_E.json) 问题"
    echo "  - Medium: $(wc -l < $WORK_DIR/ORAN-Bench-13K/Benchmark/fin_M.json) 问题"
    echo "  - Hard: $(wc -l < $WORK_DIR/ORAN-Bench-13K/Benchmark/fin_H.json) 问题"
else
    echo "✗ 基准数据未找到"
    exit 1
fi
